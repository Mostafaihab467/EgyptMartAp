...
{
    [Route("api/jpt/v2/ProductsFilter")]
    [Authorize]
    [Signture]
    [ApiController]
    public class ProductsFilterV2Controller : ControllerBase
    {
        private readonly IRepositoryWrapper _repoWrapper;

        public ProductsFilterV2Controller(IRepositoryWrapper repoWrapper)
        {
        ...
        public async Task<IActionResult> ByTitle([FromRoute] string ProductTitle, [Range(0, 255)] byte LangID = 1, [MinValidator(1)] int PageNumber = 1, [Range(1, 100)] int SizePerPage = 50)
        {
            try
            {
                var ownerID = User.Claims.FirstOrDefault(item => item.Type == ClaimTypes.NameIdentifier)?.Value ?? "0";

                var DataResult = await _repoWrapper.ProductsRepository.GetFilteredProductsByTitle(ProductTitle, LangID, PageNumber, SizePerPage, long.Parse(ownerID));
                if (DataResult.Count > 0) return Ok(CustomResponse.Success(new { DataResult[0].TotalCount, Items = DataResult.Select(item => item.GetList()) }));
                return Ok(CustomResponse.Success(DataResult));
            }
            catch (Exception ex)
