...
{
    [Route("api/v2/Reporting/AdminDashboard")]
    [Authorize]
    [Signture]
    [ApiController]
    public class AdminDashboardV2Controller(IWrapperRepository repositorys) : ControllerBase
    {
        ...
        public async Task<IActionResult> UserSubscriptionWillFinish(int WillFinishDays = 5)
        {
            try
            {
                var response = await repositorys.AdminDashboardRepository.UserSubscriptionWillFinishV2(WillFinishDays);
        ...
        public async Task<IActionResult> UserSubscriptionCountByPlan()
        {
            try
            {
                var response = await repositorys.AdminDashboardRepository.UserSubscriptionCountByPlanV2();
                return Ok(CustomResponse.Success(response));
            }
            catch (Exception e)
            {
